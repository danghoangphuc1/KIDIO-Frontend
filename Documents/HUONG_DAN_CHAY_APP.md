...
## PHẦN 4: HƯỚNG DẪN CHẠY KIỂM THỬ TỰ ĐỘNG (UNIT TEST & WIDGET TEST)

Dự án đã được tích hợp sẵn hệ thống kiểm thử tự động (Testing) cực kỳ chuyên nghiệp ở thư mục `test/` mà **hoàn toàn không làm ảnh hưởng đến mã nguồn chính**. Nếu Thầy hỏi nhóm có làm Test không, bạn hãy tự tin biểu diễn ngay 2 phần này:

**1. Chạy Unit Test (Kiểm thử Logic Hệ thống)**
*   **Chức năng:** Kiểm tra xem `AuthProvider` xử lý trạng thái Đăng nhập thành công/Thất bại có chuẩn xác không mà không cần gọi API thật (sử dụng Mock Repository).
*   **Cách chạy:** Mở Terminal trong VS Code và gõ lệnh:
    `flutter test test/unit/auth_provider_test.dart`
*   *(Nếu terminal hiện toàn tick xanh `All tests passed!` là ăn điểm tuyệt đối).*

**2. Chạy Widget Test (Kiểm thử Giao diện)**
*   **Chức năng:** Tự động hóa việc nạp màn hình Đăng Nhập, kiểm tra xem có vẽ đúng các ô TextBox và Nút bấm không. Đồng thời tự động giả lập thao tác "bấm vào nút Đăng nhập khi để trống form" để xem app có văng lỗi "Vui lòng nhập email" đúng như kỳ vọng không.
*   **Cách chạy:** Mở Terminal gõ lệnh:
    `flutter test test/widget/login_screen_test.dart`

*(Cả 2 bài test này đều độc lập hoàn toàn với code chạy app, nên bạn có thể chạy bất cứ lúc nào, kể cả khi chưa bật Backend).*